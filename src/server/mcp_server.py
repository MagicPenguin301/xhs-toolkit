...
class PublishTask:
    ...
    task_id: str
    status: (
        str  # "pending", "uploading", "filling", "publishing", "completed", "failed"
    )
    note: XHSNote
    progress: int  # 0-100
    message: str
...
class MCPServer:
    ...
    async def _initialize_data_collection(self) -> None:
        ...
        try:
            ...
            if not cookies:
                ...
                logger.info(
                    "💡 数据采集需要登录状态，请先运行: python xhs_toolkit.py cookie save"
                )
            ...
            enable_auto_collection = (
                os.getenv("ENABLE_AUTO_COLLECTION", "false").lower() == "true"
            )
    ...
    def _setup_tools(self) -> None:
        ...
        async def test_connection() -> str:
            ...
            try:
                ...
                config_status["data_collection"] = {
                    ...
                    "auto_collection_enabled": os.getenv(
        ...
        async def smart_publish_note(
            ...
            logger.debug(
                f"📋 参数详情: images={images}, videos={videos}, topics={topics}"
            )
            ...
            try:
                ...
                # 记录解析结果
                logger.info(
                    f"✅ 智能解析结果: 图片{len(note.images) if note.images else 0}张, 视频{len(note.videos) if note.videos else 0}个, 话题{len(note.topics) if note.topics else 0}个, 合集: {note.collection}"
                )
                ...
                async_task = asyncio.create_task(
                    self._execute_publish_task(task_id, callback_url)
                )
                ...
                result = {
                    ...
                    "parsing_result": {
                        ...
                        "content_type": (
                            "图文"
                            if note.images
                            else "视频" if note.videos else "纯文本"
                        ),
                    },
                }
        ...
        async def check_task_status(task_id: str) -> str:
            ...
            if not task:
                return json.dumps(
                    {"success": False, "message": f"任务 {task_id} 不存在"},
                    ensure_ascii=False,
                    indent=2,
                )
        ...
        async def get_task_result(task_id: str) -> str:
            ...
            result = {
                ...
                "message": task.message,
                "execution_time": (
                    int(task.end_time - task.start_time)
        ...
        async def login_xiaohongshu(
            force_relogin: bool = False, quick_mode: bool = False
        ) -> str:
            """
            智能登录小红书
            ...
            """
            logger.info(
                f"🚀 MCP工具调用：智能小红书 (force_relogin={force_relogin}, quick_mode={quick_mode})"
            )
            ...
            try:
                ...
                # 使用MCP专用的智能模式
                result = await self.auth_server.smart_login(
                    interactive=False, mcp_mode=True
                )
        ...
        async def get_creator_data_analysis() -> str:
            ...
            try:
                ...
                # 读取所有数据
                dashboard_data = await csv_storage.get_latest_data(
                    "dashboard", limit=100
                )
                content_data = await csv_storage.get_latest_data(
                    "content_analysis", limit=100
                )
    ...
    async def _execute_publish_task(self, task_id: str, callback_url: str = "") -> None:
        ...
        try:
            ...
            if task.note.images or task.note.videos:
                self.task_manager.update_task(
                    task_id, status="uploading", progress=20, message="正在上传文件..."
                )
                ...
                # 没有文件的快速发布
                self.task_manager.update_task(
                    task_id, status="publishing", progress=60, message="正在发布笔记..."
                )
...
"""
    ...
    def start_stdio(self) -> None:
        ...
        try:
            ...
            if (
                cookies
                and os.getenv("ENABLE_AUTO_COLLECTION", "false").lower() == "true"
            ):
                logger.info("📊 初始化数据采集功能...")
                # stdio模式下使用无头浏览器
    ...
    async def _run_mcp_server(self):
        await self.mcp.run_async(
            transport="sse", port=self.config.server_port, host=self.config.server_host
        )
    ...
    def start(self) -> None:
        ...
        # 设置信号处理
        self._setup_signal_handlers()  # 内部只设置事件，不直接调用 async
        ...
        try:
            ...
            # 等待退出信号
            loop.run_until_complete(self.exit_event.wait())
